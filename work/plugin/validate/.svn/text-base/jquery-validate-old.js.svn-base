(function(factory) {
    ...
    var RULES = {
        ...
        isTime:function(value,errorMsg,el){
            var i= /^(\d{4})-(\d{1}|1[0-2])-(\d{1}|[12]\d{1}|3[01])$/;
            if(!i.test(value)){
                return errorMsg;
            }
        },
        optionl: function(value,errorMsg,el){
            //可选项
        },
        passwordCheck:function(value,errorMsg,el){
            var $pwd=$(el).parents('form').find('[type='+$(el).attr('type')+']');
            if($pwd.eq(0).val()!=$pwd.eq(1).val()){
                return errorMsg;
            }
        },
        toRemote:function(value,errorMsg,el){
            var state=$(el).attr('remote');
            switch (state){
                case '0':
                    return errorMsg;
                case '1':
                    return;
                default :
                    return '<img style="height: 32px;" src="'+WWWROOT+'/html/xwt/front/images/public/loading.gif">';//'验证中'
            }
        }
    };
    ...
    Validator.prototype.showMsg = function(target, msg, status, callback) {
        ...
        target.after('<span class="valid_message">' + msg + '</span>')
    };
    ...
    $.fn.validate = function() {
        ...
        if(method=='init'){
            $(this).find('.form_control').each(function(i,e){
                $(e).removeClass('error').removeClass('success').find('.valid_message').html('');
            })
        }
        if (plugin[method]) {
