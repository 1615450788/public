...
jQuery.validator.addMethod("phone", function(value, element){
	return this.optional(element) ||/^0\d{2,3}-?\d{7,8}$/.test(value)// value.isPhone();
...
jQuery.validator.addMethod("mobileAndMail", function(value, element){
	var mail=/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/,
		mobile=/(^1[3|5|7|8][0-9]{9}$)/;
	return this.optional(element) || (mail.test(value)||mobile.test(value));
}, "请输入正确的手机号或有邮箱");

jQuery.validator.addMethod("partner", function(value, element){
	var partner=/^(2088)\d{12}$/;
	return this.optional(element) || partner.test(value);
}, "以2088开头由16位纯数字组成的字符串");

jQuery.validator.addMethod("menberLength", function(value, element){
...
jQuery.validator.addMethod("next", function(value, element){
	...
	var v1 = value.replace(/\D/g,"");
	var v2 = next.value.replace(/\D/g,"");
...
jQuery.validator.addMethod("more", function(value, element,param) {
	...
	var ok = true;
	if(value!="" && otherObj && (otherObj.val().length>0)){
		var otherVal = otherObj.val();
